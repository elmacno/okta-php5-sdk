...
abstract class CacheManager
{
     ...
     * @return CacheManager
     */
    protected function setCachePool(CacheItemPoolInterface $cachePool)
    {
     ...
     * @return string
     */
    public function createCacheKey(UriInterface $uri)
    {
     ...
     */
    public function save(UriInterface $uri, \stdClass $itemToCache)
    {
        if (property_exists($itemToCache, '_links') && property_exists($itemToCache->_links, 'self') && property_exists($itemToCache->_links->self, 'href')) {
            $uri = Client::getInstance()->getDataStore()->getUriFactory()->createUri($itemToCache->_links->self->href);
        }
        $cacheItem = (new CacheItem($this->createCacheKey($uri)))->set($itemToCache)->expiresAfter(new \DateInterval('PT3600S'));
        ...
        if ($links) {
            foreach ($links as $link) {
                if (is_object($link) && property_exists($link, 'href')) {
                    $uri = Client::getInstance()->getDataStore()->getUriFactory()->createUri($link->href);
                }
                $this->cachePool->deleteItem($this->createCacheKey($uri));
            }
        }
