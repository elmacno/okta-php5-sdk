...
class ClientBuilder
{
     ...
     * @return ClientBuilder
     */
    public function setIntegrationUserAgent($integrationUserAgent)
    {
    ...
    public function build()
    {
        ...
        return new Client($this->token, $this->organizationUrl, $this->httpClient, $this->integrationUserAgent, $this->cacheManager);
    }
    /**
     ...
     * @return void
     */
    private function setOptionsBasedOnFile($configFileLocation)
    {
        $parsed = $this->yamlParser->parse(file_get_contents($configFileLocation));
        if (!key_exists('okta', $parsed) || !key_exists('client', $parsed['okta'])) {
